...
/*   Updated: 2018/07/19 15:24:05 by hmartzol         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline t_ma_link	*sif_cut(size_t nb, int type, t_ma_link *l)
{
	...
	if (type != MA_T_LARGE && nb + sizeof(t_ma_link) <= l->size)
	{
...
t_ma_link				*ma_get_space(size_t nb, int type)
{
	...
	{
		l = h->data;
		total_size = sizeof(s_ma_head);
		while (1)
		{
			if (!l->allocated && l->size >= nb)
				return (sif_cut(nb, type, l));
			if ((total_size += sizeof(s_ma_link) + l->size) >=
